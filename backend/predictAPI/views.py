    ...
    for SatID in idList:
        try:
            ...
            try:
                ...
                url = "https://celestrak.org/NORAD/elements/gp.php?CATNR={}".format(
                    SatID
                )
...
def predict(settings):
    ...
    customStartTime = datetime.strptime(
        settings["customStartTime"], "%Y-%m-%dT%H:%M:%S.%fZ"
    )
    ...
    if predictionType == "realtime":
        t0 = ts.utc(
            ts.now().utc_datetime() - timedelta(seconds=30)
        )  # Time range starts 30s before now
    else:
    ...
    for i, satloop in enumerate(satList):
        ...
        if events_temp[0] != 0:
            ...
            aosArray = np.delete(
                t_temp1,
                np.append(np.where(events_temp1 == 1), np.where(events_temp1 == 2)),
            )
        ...
        for j, _ in enumerate(t_LOS):
            minSinceEpoch = (
                ...
                - datetime.timestamp(satellites[i].epoch.utc_datetime())
            ) / 60
            orbitsSinceEpoch = np.floor(minSinceEpoch * angVel / (2 * np.pi))
            orbitnum = orbitatEpoch + orbitsSinceEpoch

            passes[passIndex] = {
                "start": datetime.timestamp(t_AOS[j].utc_datetime()),
                "end": datetime.timestamp(t_LOS[j].utc_datetime()),
                "duration": (
                    t_LOS[j].utc_datetime() - t_AOS[j].utc_datetime()
                ).total_seconds(),
                "satName": satellites[i].name,
                "NORADid": satList[satloop]["NORADid"],
                "priority": satList[satloop]["priority"],
                "satIND": i,
                "orbitnum": orbitnum,
                "take": True,
            }
    ...
    for i in range(0, len(passesSorted)):
        if passesSorted[i]["take"] == True:
            for j in range(i + 1, len(passesSorted)):
                if (
                    passesSorted[i]["end"] + minSecBetweenPass
                    > passesSorted[j]["start"]
                ):
                    if passesSorted[i]["priority"] < passesSorted[j]["priority"]:
...
def calc_map_coords(passesSorted, passIndex, username, session):
    try:
        ...
        try:
            settings = user_setting.objects.get(username=username).custom_settings
        except:
            settings = user_setting.objects.get(username="default").custom_settings
    stationCoord = [float(settings["stationLong"]), float(settings["stationLat"])]
    selected_pass = passesSorted[passIndex]
    satellite, _ = make_sats_from_id([selected_pass["NORADid"]])
    ts = load.timescale()
    AOStime = ts.from_datetime(
        datetime.utcfromtimestamp(selected_pass["start"]).replace(
            tzinfo=pytz.timezone("UTC")
        )
    )
    LOStime = ts.from_datetime(
        datetime.utcfromtimestamp(selected_pass["end"]).replace(
            tzinfo=pytz.timezone("UTC")
        )
    )
    AOSpos = wgs84.subpoint(satellite[0].at(AOStime))
    LOSpos = wgs84.subpoint(satellite[0].at(LOStime))
    AOScoord = [round(AOSpos.longitude.degrees, 2), round(AOSpos.latitude.degrees, 2)]
    LOScoord = [round(LOSpos.longitude.degrees, 2), round(LOSpos.latitude.degrees, 2)]
...
class saveToSession(APIView):
    def post(self, request):
        ...
        idList = [
            newSettings["satList"][sats]["NORADid"] for sats in request.data["satList"]
        ]
        ...
        if makeSatError is False:
            ...
            return Response(
                data={"Settings saved to session."}, status=status.HTTP_200_OK
            )
        else:
            errorString = "NORAD ID: {ErrorID} caused an error, settings not saved."
            return Response(
                data=errorString.format(ErrorID=makeSatError),
    ...
    def post(self, request):
        ...
        if makeSatError is False:
            ...
            user_setting.objects.update_or_create(
                username=request.user.username,
                defaults={"custom_settings": request.data},
            )
            return Response(
                data={"Saved settings to database."}, status=status.HTTP_200_OK
            )
        else:
            errorString = "NORAD ID: {ErrorID} caused an error, settings not saved."
            return Response(
